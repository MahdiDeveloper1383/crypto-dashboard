...
export default function MarketOverview() {
  const { data: globals, isLoading, error } = UseGlobal();
  return (
      ...
      <div className="w-full  grid grid-cols-[repeat(auto-fit,minmax(200px,1fr))] gap-6 p-5 rounded-2xl">
        ...
        {!isLoading && !error && (
          <React.Fragment>
            <MarketstateCard
              title="total market"
              value={`$${globals?.data.total_market_cap.usd.toFixed(2)}`}
              change={globals?.data.market_cap_change_percentage_24h_usd.toLocaleString()}
            />
            <MarketstateCard
              title="total volume"
              value={`$${globals?.data.total_volume.usd.toFixed(2)}`}
            />
            <MarketstateCard
              title="active coins"
              value={globals?.data.active_cryptocurrencies}
            />
